...
import com.jz.blog.repository.TagRepository;
...
/**
 * REST controller for managing {@link com.jz.blog.domain.Tag}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TagResource {
    ...
    public static class TagResourceException extends RuntimeException {
        public TagResourceException(String message) {
            super(message);
        }
    }
    ...
    public ResponseEntity<Tag> createTag(@Valid @RequestBody Tag tag) throws URISyntaxException {
        ...
        Optional<Tag> existingTag = tagRepository.findOneByName(tag.getName());
        if (existingTag.isPresent()) {
            throw new BadRequestAlertException("The tag name already exists", ENTITY_NAME, "nameexists");
        }
    ...
    public ResponseEntity<Tag> updateTag(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Tag tag)
        ...
        Optional<Tag> exitingTag = tagRepository.findOneByName(tag.getName());
        if (exitingTag.isPresent() && !Objects.equals(exitingTag.get().getId(), id)) {
            throw new BadRequestAlertException("Tag name already exists", ENTITY_NAME, "nameexists");
        }
